...
export function saveShopNewsToCache(news: ShopNews[]) {
  try {
    ...
    logInfo("repository", "Saved shop news to cache", { count: news.length });
  } catch (e) {
...
export function loadShopNewsFromCache(): ShopNews[] | null {
  try {
    ...
    logInfo("repository", "Loaded shop news from cache", { count: news.length });
    return news;
  } catch (e) {
...
export function saveEventNewsToCache(news: ShopNews[]) {
  try {
    ...
    logInfo("repository", "Saved event news to cache", { count: news.length });
  } catch (e) {
...
export function loadEventNewsFromCache(): ShopNews[] | null {
  try {
    ...
    if (!json) return null;
    const news = JSON.parse(json) as ShopNews[];
    logInfo("repository", "Loaded event news from cache", { count: news.length });
    return news;
  } catch (e) {
